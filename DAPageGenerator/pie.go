...
func pieBase() *charts.Pie {
	...
	pie.AddSeries("pie", generatePieItems("淘寶"))
	return pie
}
...
func pieShowLabel() *charts.Pie {
	...
	pie.AddSeries("pie", generatePieItems("蝦皮")).
...
func pieRoseAreaRadius() *charts.Pie {
	...
	pie.AddSeries("area", generatePieItems("蝦皮")).
		SetSeriesOptions(
			charts.WithPieChartOpts(opts.PieChart{
				Radius:   []string{"30%", "75%"},
				RoseType: "area",
...
func pieInPie() *charts.Pie {
	...
	pie.AddSeries("radius", generatePieItems("淘寶"),
		charts.WithPieChartOpts(opts.PieChart{
			Radius:   []string{"0%", "45%"},
			RoseType: "radius",
		}),
